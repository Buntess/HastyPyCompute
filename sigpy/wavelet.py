...
def fwt(input, wave_name="db4", axes=None, level=None):
    ...
    zinput = util._resize(input, zshape)
...
def iwt(input, oshape, wave_name="db4", axes=None, level=None):
    ...
    ishape, coeff_slices = get_wavelet_shape(oshape, wave_name=wave_name, axes=axes, level=level)
    ...
    output = util._resize(output, oshape)
